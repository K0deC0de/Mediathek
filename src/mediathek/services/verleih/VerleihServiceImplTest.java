...
public class VerleihServiceImplTest
{
    ...
    public VerleihServiceImplTest()
    {
        ...
        _vormerkkunde1 = new Kunde(new Kundennummer(666999), "peter", "dingens");
        _vormerkkunde2 = new Kunde(new Kundennummer(767676), "lars", "müller");
        _vormerkkunde3 = new Kunde(new Kundennummer(666777), "paul", "panter");
        _vormerkkunde4 = new Kunde(new Kundennummer(666997), "mieser", "kadser");
    ...
    @Test
    public void testeVormerkenUndAusleihenUndNachrücken() throws ProtokollierException
    {
        _service.merkeVor(_vormerkkunde1, _medienListe);
		_service.merkeVor(_vormerkkunde2, _medienListe);
		_service.merkeVor(_vormerkkunde3, _medienListe);
		
		// Schaut, ob jeder hinzugefügte Kunde an der richtigen Stelle vorgemerkt ist.
		for(Medium medium : _medienListe)
		{
			assertEquals(_vormerkkunde1, _service.gibKundeByIndex(0, medium));
			assertEquals(_vormerkkunde2, _service.gibKundeByIndex(1, medium));
			assertEquals(_vormerkkunde3, _service.gibKundeByIndex(2, medium));
		}
		_service.verleiheAn(_vormerkkunde1, _medienListe, _datum);
		
		// Schaut, ob Kunde1 Ausleihen konnte und ob der Rest nachgerückt ist.
		for(Medium medium : _medienListe)
		{
			assertFalse(_service.gibVormerkkarte(medium).istDrin(_vormerkkunde1));
			assertEquals(_vormerkkunde2, _service.gibKundeByIndex(0, medium));
			assertEquals(_vormerkkunde3, _service.gibKundeByIndex(1, medium));
			assertNull(_service.gibKundeByIndex(2, medium));
		}
    }
    ...
    public void testeIstVormerkenMoeglich()
    {
		...
		// Kunde1 ist bereits in der Liste und sollte nicht mehr hinzugefügt werden können.
		// Kunde3 ist noch nicht in der Liste und hinzufügen sollte möglich sein.
		assertFalse(_service.istVomerkungMoeglich(_vormerkkunde1, _medienListe));
		assertTrue(_service.istVomerkungMoeglich(_vormerkkunde3, _medienListe));

		_service.merkeVor(_vormerkkunde3, _medienListe);
		
		// Die Liste hat bereits 3 Vormerker und sollte somit keinen 4. Kunden aufnehmen können.
		assertFalse(_service.istVomerkungMoeglich(_vormerkkunde4, _medienListe));
