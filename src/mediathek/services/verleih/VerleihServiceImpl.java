        ...
        implements VerleihService
{
    ...
    */
    ...
    public boolean istVerleihenMoeglich(Kunde kunde, List<Medium> medien)
    {
        ...
        {
                if(gibVormerkkarte(medium).istVerleihenMoeglich(kunde))
                {
                        moeglich = true;
                }
                else
        ...
        return sindAlleNichtVerliehen(medien) && moeglich;
    }
            ...
            throws ProtokollierException
    {
        ...
        for (Medium medium : medien)
        {
            ...
            if(!gibVormerkkarte(medium).gibEmpty())
            {
                    gibVormerkkarte(medium).entferne(kunde);
            }
        }
	...
	public void merkeVor(Kunde kunde, List<Medium> medien)
	{
    	...
    	for(Medium medium : medien)
    	{
        	gibVormerkkarte(medium).fuegeHinzu(kunde);       	            
        }       
        informiereUeberAenderung();
    }

    @Override
    public boolean istVomerkungMoeglich(Kunde kunde, List<Medium> medien)
    {
    	assert kundeImBestand(kunde) : "Vorbedingung verletzt: kundeImBestand(kunde)";    
    	assert medienImBestand(medien) : "Vorbedingung verletzt: medienImBestand(medien)";
    	...
    	for(Medium medium : medien)
    	{
        	if(gibVormerkkarte(medium).istVomerkungMoeglich(kunde))
        	{
            	moeglich = true;
            }
        	else
    ...
    {
    	assert mediumImBestand(medium) : "Vorbedingung verletzt: mediunImBestand(medium)";
    ...
    @Override
    public Kunde gibKundeByIndex(int i, Medium medium)
    {
        assert i >= 0 && i < 3 : "Vorbedingung verletzt: i >= 0 && i < 3 ";
        return gibVormerkkarte(medium).gibKundeByIndex(i);
    }
    ...
    //TODO für uns zum anzeigen in der gui
    @Override
    public Kunde getEntleiherFuerVormerken(Medium medium)
    {
        if(istVerliehen(medium))
        {
            Verleihkarte verleihkarte = _verleihkarten.get(medium);
